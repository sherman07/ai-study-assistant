...
assert.ok(sourceViewer.includes("toolbar=0"), "native PDF should hide the browser print/download toolbar");
assert.ok(sourceViewer.includes("is-native-pdf"), "PDF review mode should mark the panel for borderless chrome");
...
assert.ok(css.includes("is-native-pdf"), "borderless PDF review styles should exist");
assert.ok(css.includes("top: -56px") || css.includes("top:-56px"), "Chromium PDF toolbar should be clipped when it still appears");
...
assert.ok(index.includes("style.css?v=pdf-review-clean-v1"), "styles should cache-bust");
assert.ok(index.includes("synapse-legacy-controller-combined.js?v=pdf-review-clean-v1"), "controller should cache-bust");
