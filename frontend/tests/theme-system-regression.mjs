...
for (const page of ["index.html", "landing.html", "focus-room.html", "login.html", "pricing.html"]) {
  ...
  assert.match(html, /styles\/00-theme\.css\?v=(?:theme-type-scale-v1|ui-english-v1|notes-source-priority-v1|notes-responsive-v2|notes-layout-fix-v1)/, `${page} loads the current semantic theme stylesheet`);
  assert.match(html, /styles\/99-dark-mode\.css\?v=(?:dark-mode-v6|notes-source-priority-v1|notes-source-split-v1|source-preview-instant-v1|mindmap-revert-v1|mindmap-theta-v1|notes-responsive-v2|notes-layout-fix-v1)/, `${page} loads the current dark-mode compatibility layer`);
  if (html.includes("config.js")) {
    assert.match(html, /config\.js\?v=(?:public-auth-session-v4|login-remember-v1|legacy-boot-fix-v1|signup-email-v1|credits-model-v1|credits-live-v2|notes-responsive-v2|notes-layout-fix-v1)/, `${page} loads the current runtime config`);
  }
}
