...
function isDatabaseUnavailableError(error) {
  ...
  return /Database connection unavailable/i.test(message);
}

function databaseUnavailableError(cause) {
  const error = new Error(
    "Durable account storage is temporarily unavailable. Study history may stay in this browser until MySQL or Supabase is reachable."
  );
  error.status = 503;
